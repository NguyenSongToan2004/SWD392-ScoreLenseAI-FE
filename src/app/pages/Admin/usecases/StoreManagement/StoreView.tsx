...
const StoreView = () => {
    ...
    const handleEdit = () => {
        if (store) {
            // const state = getNavigationState(loc);
            nav(`/admin/store-management/edit/${store.storeID}`, {
                state: {
                    userInfo: getNavigationState(loc, 'userInfo'),
                    store: getNavigationState(loc, 'store')
                }
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${store.status === 'activate'
                                ...
                                {store.billiardTables.map((table) => (
                                    <tr key={table.billardTableID} className="bg-white border-b">
                                        ...
                                        <td className="px-4 py-2">
                                            <span className={`px-2 py-1 rounded text-xs ${table.status === 'available' ? 'bg-green-100 text-green-800' :
